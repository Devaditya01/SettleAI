...
const money = window.SettleEngine.money;
...
function timeline(t){
  ...
  if(result.reason_code==='BANK_OUTCOME_MISSING')sorted.push({at:AS_OF,title:'Bank outcome unavailable',detail:'Unknown as of this snapshot',kind:'review',icon:'help'});
  return sorted;
}
...
function showGuide(){
  openDialog('Settlement quick tour.',`<p class="guide-intro">One place to answer the question that keeps coming back: <strong>“Where is my settlement?”</strong></p><div class="guide-step"><span class="guide-number">1</span><div><strong>Find a payment</strong><p>Search an ID or customer, filter by payment date, or open the exceptions queue.</p></div></div><div class="guide-step"><span class="guide-number">2</span><div><strong>Connect the evidence</strong><p>Select a transaction to inspect its gateway, bank, and ledger records. Open the full investigation to see every retry.</p></div></div><div class="guide-step"><span class="guide-number">3</span><div><strong>Know what’s known</strong><p>Read the confirmed outcome, inspect any uncertainty, and export a case summary for support.</p></div></div><div class="guide-scenarios"><button data-demo-case="TXN-1042">Bank rejection ↗</button><button data-demo-case="TXN-1041">Successful retry ↗</button><button data-demo-case="TXN-1046">Missing evidence ↗</button><button data-demo-case="TXN-1050">Amount mismatch ↗</button></div><p class="guide-notice"><strong>Prototype scope:</strong> 12 synthetic transactions, one demo merchant, and a fixed snapshot of 04 Sep 2026. Responses use local rules and templates. Login, merchant authorization, live integrations, database access, and an LLM are not implemented. The illustrated analyst identity is not an authenticated account.</p>`,'A QUICK TOUR');
}
